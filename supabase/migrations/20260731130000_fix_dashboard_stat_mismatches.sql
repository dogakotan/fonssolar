-- Kök neden 1: get_dashboard_summary.critical_tickets, severity IN ('kritik','yüksek')
-- ikisini birleştirip tek sayı olarak "Kritik Ticket" etiketiyle gösteriyordu —
-- Tickets listesinde bu ikisi ayrı, ayrı renkte iki seviye. Artık yalnızca
-- severity = 'kritik' sayılıyor.
...
-- Kök neden 2: get_finans_overview_internal/get_finans_overview_all_internal'daki
-- quickFacts.pendingCount/pendingAmount ("Onay Bekleyen Fatura" kartı) status IN
-- ('yönetici_onayında','duzeltme_bekliyor') kullanıyordu — Onay Kuyruğu
-- (get_invoice_approval_queue) ve Faturalar listesinin "Onay Bekleyen" sekmesi
-- yalnızca 'yönetici_onayında'yı sayıyor. Aynı RPC'de zaten doğru/dar hesaplama
-- vardı (v_ai_yonetici_count/amount, actionItems.yoneticiOnayi'nde kullanılıyordu)
-- — yalnızca quickFacts bunu kullanmıyordu. kpi.pendingCount/pendingAmount
-- (bütçe/"taahhüt edilmiş tutar" hesaplarında kullanılıyor, orada geniş tanım
-- doğru) kasıtlı olarak DEĞİŞTİRİLMEDİ.
...
begin
  ...
  result := jsonb_build_object(
    ...
    'quickFacts', jsonb_build_object(
      'pendingCount', v_ai_yonetici_count, 'pendingAmount', v_ai_yonetici_amount,
