-- Harcama Dağılımı kartı Satın Alma'nın kendi Malzeme/Hizmet ayrımına göre kalsın (kullanıcı tercihi) —
-- Maliyet Tablosu/costBuckets ise GES alt kategorilerinde (Panel/İnverter/...) kalmaya devam ediyor,
-- sadece dagilim bu 3'lü gerçek invoices.category kırılımına geri dönüyor.
...
BEGIN
  ...
  -- Harcama Dağılımı: Satın Alma'nın kendi Malzeme/Hizmet ayrımıyla tutarlı, invoices.category'den
  -- birebir gerçek 3 kova (malzeme/iscilik/diger) — GES alt kategorilerine (costByCategory) bölünmez.
  SELECT jsonb_build_object(
    'malzeme', COALESCE(SUM(amount) FILTER (WHERE category = 'malzeme'), 0),
    'iscilik', COALESCE(SUM(amount) FILTER (WHERE category = 'iscilik'), 0),
    'diger',   COALESCE(SUM(amount) FILTER (WHERE category IS NULL OR category NOT IN ('malzeme','iscilik')), 0)
  ) INTO v_dagilim
  FROM invoices WHERE project_id = p_project_id AND status IN ('onaylandı','ödendi') AND invoice_date <= p_as_of_date;

  -- GES'e özgü 6 maliyet kovası (Maliyet Tablosu / Maliyet Kalemi Özeti için) — planı budget_lines'tan
  -- birebir doğru; invoices.category yalnızca malzeme/iscilik/diger düzeyinde tutulduğu için malzeme
  -- alt kovalarının (panel/inverter/konstruksiyon/kablo_elektrik) gerçekleşen/bekleyen tutarı planlanan
  -- bütçe ağırlığına göre orantılı dağıtılır.
  WITH bucket_map AS (
    SELECT name, category, planned_amount,
      CASE
        WHEN category = 'panel' THEN 'panel'
        WHEN category = 'inverter' THEN 'inverter'
        WHEN category = 'mekanik' THEN 'konstruksiyon'
        WHEN category IN ('elektrik_dc','elektrik_ac','elektrik_og') THEN 'kablo_elektrik'
    ...
    SELECT COALESCE(SUM(planned_amount) FILTER (WHERE bucket IN ('panel','inverter','konstruksiyon','kablo_elektrik')), 0) AS total
    FROM bucket_map
  ), invoice_agg AS (
    SELECT
      COALESCE(SUM(amount) FILTER (WHERE category = 'malzeme' AND status IN ('onaylandı','ödendi')), 0) AS malzeme_actual,
      COALESCE(SUM(amount) FILTER (WHERE category = 'malzeme' AND status IN ('bekliyor','muhasebe_onayında','yönetici_onayında')), 0) AS malzeme_pending,
      COALESCE(SUM(amount) FILTER (WHERE category = 'iscilik' AND status IN ('onaylandı','ödendi')), 0) AS iscilik_actual,
      COALESCE(SUM(amount) FILTER (WHERE category = 'iscilik' AND status IN ('bekliyor','muhasebe_onayında','yönetici_onayında')), 0) AS iscilik_pending,
      COALESCE(SUM(amount) FILTER (WHERE (category = 'diger' OR category IS NULL) AND status IN ('onaylandı','ödendi')), 0) AS diger_actual,
      COALESCE(SUM(amount) FILTER (WHERE (category = 'diger' OR category IS NULL) AND status IN ('bekliyor','muhasebe_onayında','yönetici_onayında')), 0) AS diger_pending
    ...
    SELECT
      ...
      COALESCE(CASE
        WHEN k.bucket IN ('panel','inverter','konstruksiyon','kablo_elektrik') THEN ROUND(ia.malzeme_actual * (COALESCE(bp.planned,0) / NULLIF(mp.total, 0)))
        WHEN k.bucket = 'iscilik' THEN ia.iscilik_actual
        ELSE ia.diger_actual
      END, 0) AS actual,
      COALESCE(CASE
        WHEN k.bucket IN ('panel','inverter','konstruksiyon','kablo_elektrik') THEN ROUND(ia.malzeme_pending * (COALESCE(bp.planned,0) / NULLIF(mp.total, 0)))
        WHEN k.bucket = 'iscilik' THEN ia.iscilik_pending
        ELSE ia.diger_pending
      END, 0) AS pending,
      COALESCE(bp.lines, '[]'::jsonb) AS lines
    FROM (SELECT unnest(ARRAY['panel','inverter','konstruksiyon','kablo_elektrik','iscilik','diger']) AS bucket) k
    LEFT JOIN bucket_planned bp ON bp.bucket = k.bucket
    CROSS JOIN material_planned mp
    CROSS JOIN invoice_agg ia
  )
  SELECT
