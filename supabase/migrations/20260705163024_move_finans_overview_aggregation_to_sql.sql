...
BEGIN
  ...
  INTO v_total_actual, v_total_actual_incl, v_pending_count, v_pending_amount, v_this_month_actual
  ...
  WITH bucket_map AS (
    ...
    SELECT
      CASE WHEN category = 'malzeme' THEN 'malzeme' WHEN category = 'iscilik' THEN 'iscilik' ELSE 'diger' END AS bucket,
      SUM(amount) AS actual
    FROM invoices
    WHERE project_id = p_project_id AND status IN ('onaylandı','ödendi') AND invoice_date <= p_as_of_date
    GROUP BY 1
  ), bucket_planned AS (
    ...
    SELECT
      ...
      COALESCE(ba.actual, 0) AS actual,
    ...
    LEFT JOIN bucket_actual ba ON ba.bucket = k.bucket
  )
  SELECT
    jsonb_agg(jsonb_build_object(
      'key', key, 'planned', planned, 'actual', actual,
  ...
  INTO v_buckets, v_bucket_total_planned, v_bucket_total_actual, v_bucket_total_sapma, v_over_budget_count, v_savings_amount
  FROM buckets;
  ...
  result := jsonb_build_object(
    ...
    'kpi', jsonb_build_object(
      ...
      'remainingBudget', v_remaining_budget, 'usagePct', v_usage_pct, 'remainingPct', v_remaining_pct,
