CREATE OR REPLACE FUNCTION public.get_project_scope(p_project_id text DEFAULT NULL::text)
...
DECLARE
  ...
  v_cross_project boolean;
  v_own_project   text;
  v_scope_all     boolean := false;
  v_authorized    boolean := true;
  v_project_ids   text[] := '{}';
BEGIN
  SELECT COALESCE(r.is_manager, false), COALESCE(r.cross_project, false)
    INTO v_is_manager, v_cross_project
  ...
  IF p_project_id IS NOT NULL THEN
    ...
    ELSE
      ...
      v_authorized  := false;
    END IF;
  ELSIF v_is_manager OR v_cross_project THEN
    v_scope_all := true;
  ELSE
