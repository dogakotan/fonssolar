-- Retry-on-conflict for purchase_requests.request_no generation.
-- Workaround for an intermittent, unexplained duplicate-key error on
-- purchase_requests_request_no_key (root cause not found locally, reported
-- to Supabase support separately) — regardless of the underlying cause,
-- retrying with a freshly drawn request_no makes the insert self-heal
-- instead of surfacing a user-facing failure. Capped at 5 attempts so a
-- genuinely different unique_violation still surfaces instead of looping.
