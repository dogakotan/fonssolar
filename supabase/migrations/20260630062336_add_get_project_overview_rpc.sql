...
DECLARE
  v_report_ids UUID[];
BEGIN
  -- Tarih aralığındaki rapor ID'lerini topla
  ...
  RETURN jsonb_build_object(
    ...
    -- ── Günlük raporlar (creator adıyla birleşik) ─────────────────────────────
    'daily_reports', (
    ...
    -- ── Görev ilerleme haritası: task_id (text) → % ───────────────────────────
    -- Her progress_item için p_end_date'e kadar birikimli qty hesaplanır,
    -- sonra aynı task_id'ye ait item'ların ortalaması alınır.
    'task_progress', (
      SELECT COALESCE(
...
-- GRANT: Supabase authenticated kullanıcılar çağırabilsin
GRANT EXECUTE ON FUNCTION get_project_overview(TEXT, DATE, DATE) TO authenticated;
