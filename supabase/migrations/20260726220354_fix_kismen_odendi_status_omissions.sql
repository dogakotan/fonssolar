-- Kısmi ödeme (kismen_odendi) durumu invoices_status_check'e eklenmiş ama 5
-- yerde unutulmuş: (1) transition validator kısmi ödeme insert/iptalini
-- reddediyordu, (2) cost_allocations sync'i kısmen ödenmiş faturayı
-- "gerçekleşen" maliyetten siliyordu, (3-5) get_finans_overview(_all)_internal
-- ve get_dashboard_summary aynı faturayı Maliyet Tablosu/S-eğrisi/Genel
-- Bakış'ta hiç saymıyordu. Hepsi aynı eski 3 değerlik ('onaylandı',
-- 'odeme_bekliyor','ödendi') sete kismen_odendi eklenerek düzeltiliyor.
...
begin
  if old.status is distinct from new.status then
    if not (
      ...
      or (old.status = any(array['odeme_bekliyor','kismen_odendi','ödendi']) and new.status = any(array['odeme_bekliyor','kismen_odendi','ödendi']) and v_role in ('admin', 'muhasebe'))
...
begin
  ...
  select
    coalesce(sum(total_amount) filter (where status in ('onaylandı','odeme_bekliyor','kismen_odendi','ödendi') and invoice_date <= p_as_of_date), 0),
    ...
    coalesce(sum(total_amount) filter (where status in ('onaylandı','odeme_bekliyor','kismen_odendi','ödendi') and invoice_date >= date_trunc('month', p_as_of_date) and invoice_date <= p_as_of_date), 0)
  ...
  with bucket_map as (
    ...
    select
      ...
      coalesce(sum(total_amount) filter (where status in ('onaylandı','odeme_bekliyor','kismen_odendi','ödendi')), 0) as actual,
