-- Malzeme Listesi'nde değişen/yeni eklenen kalemleri satır sonunda kırmızı "!" ile
-- işaretlemek için: get_satin_alma_overview'un procurement_items payload'una has_history
-- eklendi. Üç kaynağı OR'lar: (1) bu kaleme bağlı herhangi bir manuel miktar değişikliği
-- talebi (durumu ne olursa olsun), (2) bu kaleme bağlı herhangi bir otomatik/onaylı fazla
-- satın alma eklemesi, (3) kalemin kendisi onaylı bir "yeni malzeme ekleme" talebiyle
-- oluşmuş olabilir — o talep procurement_item_id taşımaz (kalem henüz yokken açıldı),
-- bu yüzden proje + malzeme adı eşleşmesiyle aranıyor.
  ...
  SELECT jsonb_build_object(
    ...
    'procurement_items', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'has_history', (
          EXISTS (SELECT 1 FROM procurement_item_change_requests pcr WHERE pcr.procurement_item_id = pi.id)
          OR EXISTS (SELECT 1 FROM procurement_item_adjustments a WHERE a.procurement_item_id = pi.id)
          OR EXISTS (
