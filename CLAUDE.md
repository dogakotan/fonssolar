...
- `ProjeDetay.jsx`'in 8 sekmesi: Genel Proje, İş Planı, Satın Alma, Malzeme
  ...
  Fotoğrafları (Ticketlar/Riskler `slice(0,5)`). Risk severity frontend'de
  kural bazlı normalize edilir: `gorev_gecikmesi` 7+ gün gecikmişse `kritik`,
  azsa `orta`; `malzeme_fazla_talep` `yüksek`; diğerlerinde kayıttaki severity.
  Proje Excel içe/dışa aktar yalnızca Proje Yönetimi sayfasında; proje
...
ile de otomatik oluşur/kapanır: (1) plan bitiş tarihi geçmiş + tamamlanmamış
görev → şiddet gecikme gün sayısına göre; (2) bir BOM kalemi için satın alma
talepleri toplamı planlanan miktarı aşarsa. Tetikleyiciler `project_tasks`/
`purchase_requests`/`purchase_request_items`/`daily_reports` üzerinde.
`project_risks` kolonları: `source` (`manuel`/`otomatik`), `rule_code`
(`gorev_gecikmesi`/`malzeme_fazla_talep`/null), `subject_ref`, `category`
(`is_kalemi`/`satin_alma`/`diger`). Koşul düzelince risk kendiliğinden
`kapatıldı` olur — elle kapatma butonu yok. Risklerin göründüğü yer Genel
Proje sekmesindeki "Açık Riskler (Detay)" kartı; `source==='otomatik'` olanlar
"Sistem tarafından tespit edildi" rozeti taşır, satıra tıklamak `rule_code`'a
göre İş Planı/Satın Alma sekmesine götürür.
