...
export default function MaliyetTablosu() {
  ...
  useEffect(() => {
    async function load() {
      const [sumRes, invRes, budRes, alcRes] = await Promise.all([
  ...
  const totalPlanned  = summary.reduce((s, r) => s + (r.planned_total || 0), 0)
  const totalActual   = summary.reduce((s, r) => s + (r.actual_total  || 0), 0)
  const totalVariance = totalPlanned - totalActual
  const avgSpendPct   = summary.length
    ? Math.round(summary.reduce((s, r) => s + (r.spend_pct || 0), 0) / summary.length)
    : 0
  ...
  const catRows = categories.map(cat => {
    ...
    const actual  = allocations.filter(a => a.category === cat).reduce((s, a) => s + (a.amount || 0), 0)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {catRows.map(r => {
                ...
                return (
                  <tr key={r.cat} style={{ borderBottom: '1px solid #F3F4F6' }}>
                    ...
                    <td style={{ padding: '14px 16ans', fontSize: 14, color: '#111827' }}>{formatCurrency(r.actual)}</td>
      ...
      {catRows.length === 0 && invoices.length === 0 && (
        <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 40, textAlign: 'center', color: '#6B7280', fontSize: 14 }}>
          Maliyet verisi bulunamadı. Supabase'de budget_lines ve cost_allocations tablolarını doldurun.
        </div>
      )}
    </div>
  )
