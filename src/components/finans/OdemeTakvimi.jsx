  ...
  return (
    <div className="payment-calendar-backdrop">
      <div className="payment-calendar">
        <header><div><small>Finans / Ödeme Takibi / Ödeme Takvimi</small><h2>Ödeme Takvimi</h2><p>Fatura vadelerini ve yaklaşan ödemeleri takvim üzerinden takip edin.</p></div><button className="close" onClick={onClose}>×</button><div className="calendar-view-toggle"><button className={view === 'month' ? 'active' : ''} onClick={() => setView('month')}>▣ Ay</button><button className={view === 'week' ? 'active' : ''} onClick={() => setView('week')}>▣ Hafta</button><button className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}>☷ Liste</button></div></header>
