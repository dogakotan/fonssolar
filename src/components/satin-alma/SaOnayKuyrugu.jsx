...
function ItemsPanel({ items }) {
  if (!items?.length) return (
    <tr>
      <td colSpan={8} style={{ padding: '10px 24px', background: '#F9FAFB', fontSize: 13, color: '#9CA3AF' }}>Kalem bilgisi yok.</td>
    </tr>
  )
  return (
    <tr>
      <td colSpan={8} style={{ padding: '0 24px 12px', background: '#F9FAFB', borderBottom: '2px solid #E5E7EB' }}>
...
export default function SaOnayKuyrugu() {
  const { user } = useAuth()
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #E5E7EB' }}>
              {['', 'TALEP BAŞLIĞI', 'PROJİ', 'ACİLİYET', 'KALEMLER', 'TALEP EDEN', 'TARİH', 'İŞLEM'].map(h => (
          ...
          <tbody>
            {requests.map(r => {
              ...
              return (
                <Fragment key={r.id}>
                  <tr
                    style={{ borderBottom: isOpen ? 'none' : '1px solid #F3F4F6', background: isOpen ? '#F9FAFB' : 'transparent', cursor: 'pointer' }}
                  >
                    ...
                    <td onClick={() => toggle(r.id)} style={{ padding: '12px 16px', fontSize: 13, color: '#374151' }}>{r.projects?.name || '—'}</td>
