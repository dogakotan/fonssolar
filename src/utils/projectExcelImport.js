...
// "İş Kalemleri" sayfasını parse et → project_tasks satırları
...
// "İlerleme Kalemleri" sayfasını parse et → progress_items satırları
export async function parseIlerlemeBilgileri(file) {
  const wb = await readWorkbook(file)
  const sheetName =
    wb.SheetNames.find(n => /ilerleme|progress|kalem/i.test(n)) ||
    wb.SheetNames[Math.min(1, wb.SheetNames.length - 1)]
  const raw = XLSX.utils.sheet_to_json(wb.Sheets[sheetName], { defval: '' })

  const VALID_UNITS = ['adet', 'm', 'm²', 'm³', 'kg', 'ton', 'rulo', 'kutu']
  const rows = raw
    .map((r, i) => {
      const rawUnit = String(pick(r, 'birim', 'unit') || 'adet').trim().toLowerCase()
      return {
        _id:            Date.now() + i,
        category:       normCat(pick(r, 'kategori', 'category'), 'mekanik'),
        name:           String(pick(r, 'kalem adı', 'kalem adi', 'name', 'ad', 'kalem', 'ilerleme kalemi') || '').trim(),
        unit:           VALID_UNITS.includes(rawUnit) ? rawUnit : 'adet',
...
export function downloadProjectTemplate() {
  ...
  // Sayfa 1: İş Kalemleri
  const ws1 = XLSX.utils.aoa_to_sheet([
    ['Görev Kodu', 'Görev Adı', 'Kategori', 'Alt Kategori / Kurum', 'Plan Başlangıç', 'Plan Bitiş', 'Sorumlu', 'Ekip Sayısı', 'Ekipman', 'Notlar'],
    ['T001', 'Mobilizasyon ve Şantiye Kurulumu', 'mobilizasyon', '',   '2026-07-01', '2026-07-07', 'Proje Müdürü', 10,  '', ''],
    ['T002', 'Panel Temel Kazık Çakımı',         'mekanik',       '',   '2026-07-08', '2026-07-30', 'Mekanik Şef', 25,  'Kazık çakma makinesi', ''],
    ['T003', 'Solar Panel Montajı',               'mekanik',       '',   '2026-07-20', '2026-08-15', 'Mekanik Şef', 30,  '', ''],
    ['T004', 'DC Kablo Döşeme',                   'elektrik_dc',   '',   '2026-08-01', '2026-08-20', 'Elektrik Şefi', 15, '', ''],
    ['T005', 'AC Kablo ve Pano',                  'elektrik_ac',   '',   '2026-08-10', '2026-08-25', 'Elektrik Şefi', 12, '', ''],
    ['T006', 'Devreye Alma',                      'devreye_alma',  '',   '2026-09-01', '2026-09-10', 'Proje Müdürü', 8,  '', ''],
  ])
  ws1['!cols'] = [8, 28, 15, 18, 15, 15, 15, 10, 20, 20].map(w => ({ wch: w }))
  XLSX.utils.book_append_sheet(wb, ws1, 'İş Kalemleri')

  // Sayfa 2: İlerleme Kalemleri
  const ws2 = XLSX.utils.aoa_to_sheet([
    ['Kategori', 'Kalem Adı', 'Birim', 'Hedef Miktar'],
    ['mekanik',     'Panel Montajı',         'adet', 3000],
    ['mekanik',     'Kazık Çakımı',          'adet', 2500],
    ['mekanik',     'Konstrüksiyon Montajı', 'adet', 2500],
    ['elektrik_dc', 'DC Kablo Döşeme',       'm',    15000],
    ['elektrik_ac', 'AC Kablo Döşeme',       'm',    3000],
    ['topraklama',  'Topraklama Hattı',      'm',    5000],
  ])
  ws2['!cols'] = [15, 28, 8, 15].map(w => ({ wch: w }))
  XLSX.utils.book_append_sheet(wb, ws2, 'İlerleme Kalemleri')
