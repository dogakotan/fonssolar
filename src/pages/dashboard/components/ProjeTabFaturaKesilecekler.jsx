...
function FaturaBaglaModal({ request, onClose, onLinked }) {
  ...
  async function handleLink() {
    ...
    await supabase
      ...
      .update({ status: 'fatura_kesildi', invoice_id: selectedId || null, updated_at: new Date().toISOString() })
      .eq('id', request.id)
    setSaving(false)
    onLinked()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, width: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #E5E7EB', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button onClick={handleLink} disabled={saving} style={{ background: '#185FA5', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 22px', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>
...
export default function ProjeTabFaturaKesilecekler({ projectId }) {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #E5E7EB' }}>
              {['TALEP BAŞLIĞI', 'ACİLİYET', 'TOPLAM TUTAR', 'ONAY TARİHİ', 'İŞLEM'].map(h => (
