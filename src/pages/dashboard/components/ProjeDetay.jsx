          ...
          {(() => {
            ...
            return (
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                {isModified && (
                  <button
                    onClick={() => { setFilterDate(new Date().toISOString().split('T')[0]); setFilterMode('gunluk') }}
                    title="Bugüne dön"
                    style={{
                ...
                {showCal && (
                  <div ref={calRef} style={{ position: 'fixed', top: calPos.top, right: calPos.right, zIndex: 9999 }}>
                    <DateNavigator
                      selectedDate={new Date(filterDate + 'T00:00:00')}
                      onChange={d => { setFilterDate(d.toISOString().split('T')[0]); setShowCal(false) }}
                    />
                  </div>
                )}
              </div>
