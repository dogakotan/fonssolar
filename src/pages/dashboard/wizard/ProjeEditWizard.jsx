...
export default function ProjeEditWizard({ project, onSuccess, onViewProject }) {
  ...
  function handleStepDone(stepNo, result) {
    ...
    if (actionRef.current === 'next') goNext()
  }
    ...
    <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
      <div className="card" style={{ width: 210, flexShrink: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '0.875rem', borderTop: '1px solid var(--color-border-md)', display: 'grid', gap: '0.5rem' }}>
          <button type="button" onClick={onSuccess} style={{ padding: '0.5rem', background: 'transparent', color: 'var(--color-muted)', border: '1px solid var(--color-border-md)', borderRadius: 'var(--radius-md)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>İptal</button>
          {step < 8 && <button type="button" onClick={() => submitCurrentStep('save')} style={{ padding: '0.5rem', background: '#fff', color: 'var(--color-primary)', border: '1px solid var(--color-primary)', borderRadius: 'var(--radius-md)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Kaydet</button>}
          <button type="button" onClick={() => submitCurrentStep('next')} style={{ padding: '0.5rem', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{step === 8 ? 'Kaydet' : 'Devam →'}</button>
        </div>
      </div>
